...
static int
upstream_net_key(struct upstream *u, char *key, size_t max_size) {
    size_t i, s;
    char ch;
    char name[MAX_HOSTNAME_LEN];
    const char delimiter = '.';

    ASSERT(u->server.family == AF_INET);
...
static void
upstream_net_init(struct upstream_net *n, uint8_t net_class) {
    string_init(&n->net);
    n->net_class = net_class;
    queue_null(&n->timewheel);
}

static void
upstream_net_deinit(struct upstream_net *n) {
    string_deinit(&n->net);
    if (!queue_is_null(&n->timewheel)) {
        queue_deinit(&n->timewheel);
    }
}
...
static rps_ts_t
upstream_deadline(uint32_t mr1m, uint32_t mr1h, uint32_t mr1d) {
    rps_ts_t now, m_ago, h_ago, d_ago, deadline;
...
static rps_status_t
upstream_timewheel_init(rps_queue_t *timewheel, uint32_t mr1m, uint32_t mr1h, uint32_t mr1d) {
    uint32_t n;
    ...
    return queue_init(timewheel, n);
}

static bool
upstream_timewheel_freshly(rps_queue_t *timewheel) {
    return queue_is_null(timewheel);
}

static bool
upstream_timewheel_enabled(uint32_t mr1m, uint32_t mr1h, uint32_t mr1d) {
    return (mr1m > 0 || mr1h > 0 || mr1d >0);
}

static bool
upstream_timewheel_overload(rps_queue_t *timewheel, uint32_t mr1m, uint32_t mr1h, uint32_t mr1d) {
    uint32_t m, h, d;   
    uint32_t i, j, n;
    rps_ts_t now, m_ago, h_ago;
    rps_ts_t ts;
    rps_ts_t deadline;
    ...
    deadline = upstream_deadline(mr1m, mr1h, mr1d);
...
static void
upstream_timewheel_add(rps_queue_t *timewheel) {
    rps_ts_t now;
    
    now = rps_now();

    queue_en(timewheel, (void *)now);
}
...
upstream_pool_init(struct upstream_pool *up, struct config_upstream *cu, 
    ...
    if (hashmap_init(&up->c_nets, UPSTREAM_DEFAULT_NETS_LENGTH, HASHMAP_DEFAULT_COLLISIONS) != RPS_OK) {
        return RPS_ERROR;       
    }
...
static void
upstream_net_deinit_foreach(void *data) {
    struct upstream_net *n;
    
    ASSERT(data != NULL);

    n = (struct upstream_net *)data;
    
    upstream_net_deinit(n);
}
...
upstream_pool_deinit(struct upstream_pool *up) {
    ...
    hashmap_foreach2(&up->c_nets, (hashmap_foreach2_t)upstream_net_deinit_foreach);
    hashmap_deinit(&up->c_nets);
...
upstreams_init(struct upstreams *us, struct config_api *capi, 
    ...
    us->mr1h = cus->mr1h;
    us->mr1d = cus->mr1d;
    us->c_mr1m = cus->c_mr1m;
    us->c_mr1h = cus->c_mr1h;
    us->c_mr1d = cus->c_mr1d;
...
static rps_status_t
upstream_net_merge(rps_hashmap_t *nets, rps_hashmap_t *pool) {
    uint32_t i;
    size_t key_size;
    char n_key[UPSTREAM_KEY_MAX_LENGTH];
    struct upstream *u;
    struct upstream_net *n;
    ...
    for (i = 0; i < pool->size; i++) {
        ...
        while (e != NULL) {
            ...
            //Only IPV4 address support
            if (u->server.family != AF_INET) {
                continue;
            }
            key_size = upstream_net_key(u, n_key, UPSTREAM_KEY_MAX_LENGTH);
            if (!hashmap_has(nets, n_key, key_size)) {
                if ((n = rps_alloc(sizeof(struct upstream_net))) == NULL) {
                    return RPS_ENOMEM;
                }
                upstream_net_init(n, up_net_c);
                hashmap_set(nets, n_key, key_size, &n, sizeof(n));
            }
            e = e->next;
        }
    }
...
static uint32_t
upstream_pool_available_count(rps_hashmap_t *pool) {
    uint32_t i, n;
    struct hashmap_entry *e;
    struct upstream *u;
...
static void
upstream_pool_cleanup(rps_hashmap_t *pool, uint32_t mr1m, uint32_t mr1h, uint32_t mr1d) {
    uint32_t i;
    rps_ts_t now, deadline;
    ...
    for (i = 0; i < pool->size; i++) {
        ...
        while (e != NULL) {
            u = (struct upstream *)*(void **)e->value;
            if (rps_unresolve_addr(&u->server, name) != RPS_OK) {
                e = e->next;
                continue;
            }
            ...
            if (u->expire_date > deadline) {
                u->enable = 0;
                e = e->next;
                log_debug("%s:%d will not be cleanup, expire_date:%ld, deadline:%d, now:%ld ", 
                        name, rps_unresolve_port(&u->server), u->expire_date, deadline, now);
                continue;
            }
            ...
            log_debug("%s:%d be cleanup, expire_date:%ld, deadline:%d, now:%ld (s:%d, f:%d, c:%d)", 
                    name, rps_unresolve_port(&u->server), u->expire_date, deadline, now, 
...
static rps_status_t
upstream_pool_refresh(struct upstream_pool *up, struct upstreams *us) {
    ...
    if (upstream_pool_merge(&up->pool, &new_pool) != RPS_OK) {   
        uv_rwlock_wrunlock(&up->rwlock);
        goto error;
    }

    if (upstream_timewheel_enabled(us->c_mr1m, us->c_mr1h, us->c_mr1d)) {
        if (upstream_net_merge(&up->c_nets, &new_pool) != RPS_OK) {
            uv_rwlock_wrunlock(&up->rwlock);
            goto error;
        }
    }
    
    upstream_pool_cleanup(&up->pool, us->mr1m, us->mr1h, us->mr1d);
...
upstreams_refresh(uv_timer_t *handle) {
    ...
    for (i=0; i< len; i++) {
        ...
        if (upstream_pool_refresh(up, us) != RPS_OK) { 
            log_error("update %s upstream proxy pool failed", proto) ;
            return;
        } else {
            log_info("refresh %s upstream pool, get <%d> proxys, <%d> available, <%d> C Class net", 
                    proto, hashmap_n(&up->pool), upstream_pool_available_count(&up->pool), hashmap_n(&up->c_nets));
        }
    }
...
upstreams_get(struct upstreams *us, rps_proto_t proto) {
    ...
    for ( ; ; ) {
        ...
        if (upstream_timewheel_enabled(us->mr1m, us->mr1h, us->mr1d)) {
            if (upstream_timewheel_freshly(&upstream->timewheel)) {
                upstream_timewheel_init(&upstream->timewheel, us->mr1m, us->mr1h, us->mr1d);
            }

            // upstream request too often
            if (upstream_timewheel_overload(&upstream->timewheel, us->mr1m, us->mr1h, us->mr1d)) {
                continue;
            }
        }

        if (upstream_timewheel_enabled(us->c_mr1m, us->c_mr1h, us->c_mr1d)) {
            // NON-IPV4 upstream, return directly
            if (upstream->server.family != AF_INET) {
                break;
            }

            key_size = upstream_net_key(upstream, n_key, UPSTREAM_KEY_MAX_LENGTH);
            ov = hashmap_get(&up->c_nets, n_key, key_size, &val_size);
            if (ov == NULL) {
                break;
            }

            net = (struct upstream_net *)*(void **)ov;

            if (upstream_timewheel_freshly(&net->timewheel)) {
                upstream_timewheel_init(&net->timewheel, us->c_mr1m, us->c_mr1h, us->c_mr1d);
                break;
            }

            // netblock request too often
            if (upstream_timewheel_overload(&net->timewheel, us->c_mr1m, us->c_mr1h, us->c_mr1d)) {
                continue;
            }
        }
    ...
    if (upstream != NULL) {
        upstream->count += 1;    
        if (upstream_timewheel_enabled(us->mr1m, us->mr1h, us->mr1d)) {
            upstream_timewheel_add(&upstream->timewheel);
        }
    }

    if (net != NULL) {
        if (upstream_timewheel_enabled(us->c_mr1m, us->c_mr1h, us->c_mr1d)) {
            upstream_timewheel_add(&net->timewheel);
        }
    }
