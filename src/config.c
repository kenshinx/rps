...
config_upstreams_init(struct config_upstreams *upstreams) {
    ...
    upstreams->stats = UPSTREAM_DEFAULT_STATS;
    ...
    upstreams->c_mr1m = UPSTREAM_DEFAULT_MR1M;
    upstreams->c_mr1h = UPSTREAM_DEFAULT_MR1H;
    upstreams->c_mr1d = UPSTREAM_DEFAULT_MR1D;
...
config_handler_map(struct config *cfg, rps_str_t *key, rps_str_t *val, rps_str_t *section) {
    ...
    if (section == NULL) {
        ...
        } else if (rps_strcmp(key, "stats") == 0) {
            cfg->upstreams.stats = (atoi((char *)val->data)) * 1000;
        ...
        } else if (rps_strcmp(key, "c_mr1m") == 0) { 
            cfg->upstreams.c_mr1m = atoi((char *)val->data);
        } else if (rps_strcmp(key, "c_mr1h") == 0) { 
            cfg->upstreams.c_mr1h = atoi((char *)val->data);
        } else if (rps_strcmp(key, "c_mr1d") == 0) { 
            cfg->upstreams.c_mr1d = atoi((char *)val->data);
...
config_dump(struct config *cfg) {
    ...
    log_debug("\t stats: %d", cfg->upstreams.stats/1000);
    ...
    log_debug("\t c_mr1m: %d", cfg->upstreams.c_mr1m);
    log_debug("\t c_mr1h: %d", cfg->upstreams.c_mr1h);
    log_debug("\t c_mr1d: %d", cfg->upstreams.c_mr1d);
